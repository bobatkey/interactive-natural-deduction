...
struct
  let attributes attrs =
    if not (String.Map.is_empty attrs) then begin
      attrs |> String.Map.iter begin fun name value ->
        Output.string " ";
        Output.string name;
        if value <> "" then begin
          Output.string "=\"";
...
let attributes list =
  List.fold_left
    (fun attributes (Attr (name, value)) ->
       String.Map.add name value attributes)
    String.Map.empty
    list
